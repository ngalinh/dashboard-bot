  ...
  const bots = data.bots
    ...
    .map((b) => {
      ...
      return {
        ...
        // Chat chỉ mở được khi admin đang bật + bot thực sự running
        chatUrl: enabled && b.status === 'running' && b.id ? `${base}/b/${b.id}/index.html` : null,
...
/**
 * Re-clone bot đang dùng `gitUrl` đã lưu, đổi sang `branch` mới (hoặc giữ branch cũ).
 * Body JSON: { branch? }.  Yêu cầu bot đã có `gitUrl` từ lần clone trước.
 */
router.post(
  '/bots/:id/pull',
  requirePlatformAuth,
  ...
  async (req, res) => {
    ...
    const branch = typeof body.branch === 'string' ? body.branch.trim() : '';
    const bot = req.platformBot;
    const gitUrl = bot && bot.gitUrl ? String(bot.gitUrl).trim() : '';
    if (!gitUrl) {
      return res.status(400).json({
        success: false,
        message: 'Bot này chưa được clone từ GitHub — không có gitUrl đã lưu để pull lại.',
      });
    }
    try {
      const { id } = await service.provisionExistingBotFromGit({
        id: req.params.id,
        gitUrl,
        branch: branch || (bot.gitBranch || ''),
      });
      res.status(202).json({
        success: true,
        message: branch
          ? `Đang pull lại từ branch ${branch}.`
          : 'Đang pull lại từ branch hiện tại.',
        id,
      });
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
