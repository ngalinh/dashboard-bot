...
async function ensureSchema() {
  ...
  // Migration: add dept / dept_color / role / tasks columns for the new dashboard UI
  const [cols3] = await pool.query(
    `SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
     WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'platform_bots'
       AND COLUMN_NAME IN ('dept','dept_color','role','tasks')`
  );
...
async function loadCache() {
  ...
  const [rows] = await pool.query(
    'SELECT id, name, description, allowed_role_names, dept, dept_color, role, tasks, status, port, work_dir, status_message, created_bot_at, updated_at FROM platform_bots ORDER BY created_bot_at ASC'
  );
...
async function addBot(bot) {
  ...
  await pool.execute(
    `INSERT INTO platform_bots (id, name, description, allowed_role_names, dept, dept_color, role, tasks, status, port, work_dir, status_message, created_bot_at, updated_at)
     VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
    [
      bot.id,
      bot.name,
...
async function updateBot(id, patch) {
  ...
  await pool.execute(
    `UPDATE platform_bots SET name=?, description=?, allowed_role_names=?, dept=?, dept_color=?, role=?, tasks=?, status=?, port=?, work_dir=?, status_message=?, updated_at=CURRENT_TIMESTAMP(3) WHERE id=?`,
    [
      merged.name,
      desc,
      roles.length ? JSON.stringify(roles) : null,
