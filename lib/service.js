...
/**
 * Cập nhật tên và/hoặc mô tả hiển thị trong registry.
 * @param {string} id
...
async function updateBotProfile(id, fields) {
  ...
  if (!Object.keys(patch).length) {
    return { ok: false, message: 'Không có trường cập nhật (name hoặc description)' };
  }
...
async function runBotHealthWatchdogTick() {
  ...
  const data = registry.readRegistry();
  const bots = data.bots.filter((b) => b.status === 'running' && b.port && b.workDir);
