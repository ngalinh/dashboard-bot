...
async function pm2StartPython(pyId, repoDir, venvDir, entrypoint) {
  ...
  const args = ['start', python, '--name', name, '--cwd', repoDir, '--time', '--', entrypoint];
...
async function pm2Start(botId, workDir, port) {
  ...
  const pm2Args = ['start', 'npm', '--name', name, '--cwd', workDir, '--time'];
...
function normalizePublicGithubCloneUrl(input) {
  return parseGithubCloneTarget(input).cloneUrl;
}

/**
 * Parse một URL GitHub bất kỳ (web UI hoặc clone URL) → trả về cloneUrl chuẩn
 * + branchHint nếu URL có dạng `/owner/repo/tree/<branch>` (web UI khi user copy
 * thẳng từ thanh địa chỉ trình duyệt).  Branch có thể chứa dấu `/`
 * (vd `feat/foo`) — gộp toàn bộ phần sau `tree`.
 *
 * @param {string} input
 * @returns {{ cloneUrl: string, branchHint: string }}
 */
function parseGithubCloneTarget(input) {
...
/**
 * Tải source zip vào một bot đã tồn tại (điển hình: bot placeholder đang chờ mã).
 * Giữ nguyên metadata (name/dept/role/tasks/icon); chỉ đổi workDir/port/status.
 *
 * @param {string} id
 * @param {string} zipPath đường dẫn file zip tạm (sẽ xoá sau khi xử lý)
 * @returns {Promise<{ id: string }>}
 */
async function provisionExistingBot(id, zipPath) {
  ensureDirs();
  ...
  if (!bot) {
    ...
    throw new Error('Không tìm thấy bot');
  }
  await registry.updateBot(id, { status: 'pending', statusMessage: '' });
  setImmediate(() => {
    runProvision(id, zipPath).catch(async (e) => {
      console.error('[platform] runProvision (existing)', id, e);
      await registry.updateBot(id, { status: 'error', statusMessage: e.message || String(e) });
    });
  });
  return { id };
...
async function enqueueProvisionFromGit(opts) {
  const parsed = parseGithubCloneTarget(opts.gitUrl);
  const branch = (opts.branch && String(opts.branch).trim()) || parsed.branchHint || '';
  assertBranchName(branch);
  const cloneUrl = withGithubPatForClone(parsed.cloneUrl);
  ensureDirs();
...
/**
 * Clone GitHub vào một bot đã tồn tại (điển hình: placeholder đang chờ mã).
 * Giữ nguyên metadata (name/dept/role/tasks/icon); chỉ đổi workDir/port/status.
 *
 * @param {{ id: string, gitUrl: string, branch?: string }} opts
 * @returns {Promise<{ id: string }>}
 */
async function provisionExistingBotFromGit(opts) {
  const id = opts.id;
  const bot = registry.getBot(id);
  if (!bot) throw new Error('Không tìm thấy bot');
  const parsed = parseGithubCloneTarget(opts.gitUrl);
  const branch = (opts.branch && String(opts.branch).trim()) || parsed.branchHint || '';
  assertBranchName(branch);
  const cloneUrl = withGithubPatForClone(parsed.cloneUrl);
  ...
  setImmediate(() => {
    runProvisionFromGit(id, cloneUrl, branch).catch(async (e) => {
      console.error('[platform] runProvisionFromGit (existing)', id, e);
      await registry.updateBot(id, { status: 'error', statusMessage: e.message || String(e) });
    });
  });
  return { id };
...
/**
 * Resurrect bot: trên mỗi lần platform khởi động (container restart, deploy, OOM…)
 * PM2 daemon trong container đã chết theo, nên tất cả bot process biến mất dù
 * registry vẫn ghi status='running'. Watchdog sẽ tự khởi động lại nhưng phải đợi
 * 2 chu kỳ probe (~90s) + cooldown 120s/bot, rất chậm khi có nhiều bot.
 *
 * Hàm này quét registry ngay khi platform start: bot Node nào có workDir+port và
 * không bị admin tắt thủ công (enabled !== false) → pm2Start lại; bot Python
 * tương tự nhưng cần venv + entrypoint. Lỗi 1 bot không cản các bot khác.
 */
async function ensureBotsRunning() {
  const results = { node: { started: 0, skipped: 0, failed: 0 }, python: { started: 0, skipped: 0, failed: 0 } };
  let nodeBots;
  try {
    nodeBots = registry.readRegistry().bots;
  } catch (e) {
    console.error('[platform] ensureBotsRunning: đọc registry node thất bại:', e.message || e);
    nodeBots = [];
  }
  for (const bot of nodeBots) {
    if (!bot || !bot.id) continue;
    if (bot.enabled === false) {
      results.node.skipped += 1;
      continue;
    }
    if (!bot.workDir || !bot.port) {
      results.node.skipped += 1;
    ...
    const envCheck = checkBotEnvReadyForStart(bot.id);
    if (!envCheck.ok) {
      results.node.skipped += 1;
      console.warn(`[platform] ensureBotsRunning: bỏ qua bot ${bot.id} — env chưa sẵn sàng (${envCheck.message || ''})`);
      continue;
    }
    try {
      if (await pm2ProcessExists(bot.id)) {
        results.node.skipped += 1;
        continue;
      }
      await pm2Start(bot.id, bot.workDir, bot.port);
      await registry.updateBot(bot.id, { status: 'running', statusMessage: '' });
      results.node.started += 1;
      console.log(`[platform] ensureBotsRunning: pm2 start bot ${bot.id} (port ${bot.port})`);
    } catch (e) {
      results.node.failed += 1;
      const msg = e instanceof Error ? e.message : String(e);
      console.error(`[platform] ensureBotsRunning: bot ${bot.id} pm2 start lỗi: ${msg}`);
      try {
        await registry.updateBot(bot.id, { status: 'error', statusMessage: `pm2 start (resurrect): ${msg}` });
      } catch {
        /* */
      }
    }
  ...
  let pyBots;
  try {
    pyBots = pyRegistry.list();
  } catch (e) {
    console.error('[platform] ensureBotsRunning: đọc registry python thất bại:', e.message || e);
    pyBots = [];
  }
  for (const bot of pyBots) {
    if (!bot || !bot.id) continue;
    const dirs = pyBotDirs(bot.id);
    if (!fs.existsSync(dirs.repo) || !fs.existsSync(dirs.venv)) {
      results.python.skipped += 1;
      continue;
    }
    const entrypoint = bot.entrypoint || 'bot.py';
    try {
      const name = pm2NamePy(bot.id);
      const { stdout } = await execFileAsync('pm2', ['jlist'], {
        maxBuffer: 32 * 1024 * 1024,
        env: process.env,
      });
      const list = JSON.parse(stdout);
      const exists = Array.isArray(list) && list.some((p) => {
        const n = p && (p.name || (p.pm2_env && p.pm2_env.name));
        return n === name;
      });
      if (exists) {
        results.python.skipped += 1;
      ...
      await pm2StartPython(bot.id, dirs.repo, dirs.venv, entrypoint);
      await pyRegistry.update(bot.id, { status: 'running', statusMessage: '' });
      results.python.started += 1;
      console.log(`[platform] ensureBotsRunning: pm2 start python bot ${bot.id}`);
    } catch (e) {
      results.python.failed += 1;
      const msg = e instanceof Error ? e.message : String(e);
      console.error(`[platform] ensureBotsRunning: python bot ${bot.id} pm2 start lỗi: ${msg}`);
      try {
        await pyRegistry.update(bot.id, { status: 'error', statusMessage: `pm2 start (resurrect): ${msg}` });
      } catch {
        /* */
      }
    }
  ...
  console.log(
    `[platform] ensureBotsRunning: node started=${results.node.started} skipped=${results.node.skipped} failed=${results.node.failed}; python started=${results.python.started} skipped=${results.python.skipped} failed=${results.python.failed}`
  );
  return results;
}
...
module.exports = {
  ensureDirs,
  enqueueProvision,
  provisionExistingBot,
  enqueueProvisionFromGit,
  provisionExistingBotFromGit,
  deleteBot,
  restartBot,
  setBotEnabled,
  reloadBot,
